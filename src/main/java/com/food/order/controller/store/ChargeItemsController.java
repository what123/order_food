 ...
 */
@Api( value = "分店充值选项", description = "分店充值选项的接口",tags="分店-充值选项")
@RestController("store_charge_item_controller")
@RequestMapping("/api/store/charge_item")
@CrossOrigin
public class ChargeItemsController extends BaseController {
    ...
    public MsgVo add(@RequestParam(value = "id",defaultValue = "0")Long id,
        ...
        if(id > 0){
            ...
            if(chargeItem == null || chargeItem.getStore().getId() != store.getId()){
                msgVo.setCode(40002);
                msgVo.setMsg("充值项不存在");
                return msgVo;
            }
