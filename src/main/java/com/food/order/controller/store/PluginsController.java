...
@Api( value = "插件购买管理", description = "插件购买接口",tags="分店-插件购买接口")
@Controller("store_plugins")
@RequestMapping("/api/store/plugins")
@CrossOrigin
public class PluginsController extends BaseController{
    ...
    public MsgVo list(@RequestParam(value = "keyword",defaultValue = "")String keyword,
        ...
        List<Plugins> pluginsList = pluginsServiceImpl.getStoreAllPlugins(store, 0);
        ...
        int vip = store.getMainStore().getVip();
        ...
        if(pluginsList.size() == 0){
            ...
            for(PluginsService pluginsService:pluginServices){
                ...
                if(pluginsMap.containsKey(pluginsService.getPluginsUUID())){
                    ...
                    pluginsServiceImpl.setPluginPrices(null,store,plugins,pluginsService);
                }else{
                    plugins = pluginsServiceImpl.getNewPlugins(null,store,pluginsService);
                }
