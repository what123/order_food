 ...
 */
public class StoreUserAuthInterceptor implements HandlerInterceptor {
    @Autowired
...
//        System.out.println("sessionid22++++++++++++++++++++++++++++++"+sessoin.getId());
        ...
        if(user == null || user.isDelete() || user.getBelong() != 4){
        ...
        if(storeUser.getStore().getUser().getIsFreeze() == 1 || storeUser.getStore().getUser().isDelete()){
            errorHandler(response,5004,"分店帐号被冻结或删除");
            return false;
        }
        ...
        request.setAttribute("user",storeUser);
