 ...
 */
@PayTarget
public class WxNative extends BasePlugins implements PaymentService, PluginsService {
    ...
    public WxNative(){//必须要空参的构造，否则会造成反射实例异常
    ...
    private String getSandBoxKey(String mch_id,String nonce_str){
        ...
        String sign = getSign(map,"GZkuaiding4444444444444444445785");
        map.put("sign",sign);
    ...
    public PluginsData getPaySendData(HttpServletRequest request, String order_no, Integer total_fee){
        ...
        responBodyMap.put("trade_type", "NATIVE");
    ...
    public PluginsData doPay(PluginsData payData,HashMap<String, Object> map){
        ...
        //Orders orders = (Orders) payData.getData().get("order");
        //String data = new HttpClientUtil().doPostRawXml("https://api.mch.weixin.qq.com/sandboxnew/pay/unifiedorder", map2xml(responBodyMap));
        String data = new HttpClientUtil().doPostRawXml("https://api.mch.weixin.qq.com/pay/unifiedorder", map2xml(responBodyMap));
        Map<String,String>  rsMap = xml2map(data);
        try {
            if(rsMap.get("return_code").equals("SUCCESS") && rsMap.get("result_code").equals("SUCCESS")){
                map.put("qrcode","/api/common/qrCode?content="+ URLEncoder.encode(rsMap.get("code_url"),"utf-8"));
                map.put("code_url",rsMap.get("code_url"));
                //map.put("order_no",orders.getOrderNo());
                map.put("price",Double.valueOf(total_fee)/100);
                payData1.getData().put("url","pay/wx/native");
            }else{
                payData1.setCode(100);
                payData1.setMsg("微信支付native接口:"+rsMap.get("return_msg"));
            }
        } catch (Exception e) {
            e.printStackTrace();
            payData1.setCode(100);
    ...
    public PluginsData callback(HttpServletRequest request, Orders orders){
        ...
        if(dataMap.get("return_code").equals("SUCCESS") && dataMap.get("result_code").equals("SUCCESS")){
            ...
            payData.setCode(100);
            payData.setMsg("微信支付native接口:"+request.getParameter("return_msg"));
            payData.getData().put("result","fail");
            return payData;
        }
    }
    ...
    public PluginsData refund(PluginsData  payData){
        ...
        //String data = new CertHttpUtil().postData("https://api.mch.weixin.qq.com/sandboxnew/secapi/pay/refund", map2xml(responBodyMap),mchid,p12Path);//("https://api.mch.weixin.qq.com/secapi/pay/refund", map2xml(responBodyMap));
        if(data == null){
            payData1.setCode(100);
        ...
        try {
            ...
            if(rsMap.get("return_code").equals("SUCCESS")){
                if(rsMap.get("result_code").equals("SUCCESS")) {
                    ...
                    payData1.setCode(100);
                    payData1.setMsg("微信支付接口:"+rsMap.get("err_code_des"));
                }
            }else{
                payData1.setCode(100);
                payData1.setMsg("微信支付接口:"+rsMap.get("return_msg"));
            }
        } catch (Exception e) {
            payData1.setCode(100);
    ...
    public List<Integer> canPayAgentTypes() {
        ...
        types.add(AgnetTypeEnum.MOBILE_WEB.getIndex());//微信浏览器可使用
    ...
    public int getPayOrCollect() {
        return PayOrCollectEnum.Collect.getIndex();
    }
    /**
     * 生成sign
     * @param options
    ...
    public String getPluginsUUID() {
        try {
            return MD5Util.MD5Encode(WxNative.class.getName());
